...
const handler = async (req, res) => {
    try {
        ...
        if (exist) {
            ...
            const tokenSet = await Token.create({ userId: exist.id, token: userToken })
            return res.json(success("User Login Successfully", { token: tokenSet.token, id: exist.id }))
        } else {
...
createRouter.post('/email-login', wrapRequestHandler(handler))
